...
public class OneFragment extends Fragment implements AbsListView.OnScrollListener {
    View rootView;
    Thread thread;
    LinearLayout loadingimg,fragment_one;
    ListView onelist;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.one, container, false);//关联布局文件
        init();
        return rootView;
    }
    ...
    private void init() {
        ...
        loadingimg = (LinearLayout) rootView.findViewById(R.id.one_loading);
        fragment_one = (LinearLayout) rootView.findViewById(R.id.fragment_one);
        onelist = (ListView) rootView.findViewById(R.id.onelist);
        pagerhead = (TextView) rootView.findViewById(R.id.pagerhead);
        List<String> msg = new ArrayList<String>();
        for (int i=1;i<=100;i++){
            msg.add("测试00000000"+i);
        }
    ...
    private void loadmsg() {
        ...
        ObjectAnimator objectanim = ObjectAnimator.ofFloat(fragment_one, "Y", dm.heightPixels, 0);
        anim.play(objectanim);
        anim.setDuration(500);
        anim.start();
//        HuanBaoActivty huanbao = (HuanBaoActivty) getActivity();
//        huanbao.upanim();
        onelist.setOnScrollListener(this);
        rigestBroadcast();
    }
    @Override
    ...
    BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("com.wbq.two".equals(action)||"com.wbq.three".equals(action)){
                pagerhead.setVisibility(View.GONE);
                hidehead = true;
            }else if ("com.wbq.two.down".equals(action)||"com.wbq.three.down".equals(action)){
                pagerhead.setVisibility(View.VISIBLE);
                hidehead = false;
            }
        }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        switch (scrollState) {
            ...
            case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:// 是当屏幕停止滚动时
                scrollFlag = false;
                // 判断滚动到顶部
                if (onelist.getFirstVisiblePosition() == 0) {
                    if (!upglide){
                        pagerhead.setVisibility(View.VISIBLE);
                        hidehead = false;
                ...
                // 判断滚动到底部
                if (onelist.getLastVisiblePosition() == (onelist
                        .getCount() - 1)) {
                }
                break;
